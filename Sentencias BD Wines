...
--desaparecio la definición de la tabla pedido: ver el Object Browser. 
CREATE TABLE pedido(
    codigo_vino VARCHAR2(10) NOT NULL, 
...
que pasaria se carga un codigo que NO se corresponde con uno de los clientes o preveedores
que estan cargados en las otras tablas? arroja el siguente error ORA-02291: integrity constraint (WKSP_DBCURSOUTN.SYS_C00144396938) violated - parent key not found
...
CREATE TABLE categoria_proveedor (
    codigo_cate_prov VARCHAR2(2) NOT NULL CHECK (codigo_cate_prov IN ('AC', 'BA', 'NN')),
...
INSERT INTO CATEGORIA_PROVEEDOR VALUES ( 'NN', 'CALIDAD MEDIA', 100, NULL, NULL)
